...
const Button = ({ label, href = "/", className = "", theme = "outline" }) => {
  return (
    <>
      <CustomLink href={href} label={label} className={` ${theme === "light" && "bg-white! text-[#00689F]!"} border w-fit border-[#00689F50] bg-white p-2 rounded-lg text-sm flex items-center gap-x-0 hover:gap-x-2 transition-all duration-300 group ${className}`}>
